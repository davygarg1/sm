...
export default function Navbar(props) {
  ...
  const navigation = [
    { name: 'About', to: '/About', current: location.pathname === '/About' ? true : false },
    { name: 'Blogs', to: '/Blogs', current: location.pathname === '/Blogs' ? true : false },
    { name: 'Treatment', to: '/Treatment', current: location.pathname === '/Treatment' ? true : false },
      ...
      <nav className="bg-white dark:bg-gray-900 fixed w-full z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <Link href="/#" className="flex items-center space-x-3 rtl:space-x-reverse">
            ...
            <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Samarpitam</span>
          </Link>
          ...
          <div className="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
            ...
            {localStorage.getItem("token") ?
              ...
              <button type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" onClick={() => navigate("/Login")}>Get started</button>
                ...
                <li key={item.name}>
                  <Link key={item.name} to={item.to}
                    className={classNames(
                      item.current ? 'text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 md:dark:text-blue-500' : 'text-gray-900 hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700',
                      'block py-2 px-3 '
                    )}
                    aria-current="page" >
                    {item.name}
